...
class Attaker_Neymar:
    ...
    def block_passes(self, field: fld.Field, actions: list[Optional[Action]]) -> None:
        ...
        if((y - l) > (l - p)):
            actions[self.idxN] = Actions.GoToPointIgnore(r , (field.ball.get_pos() - field.allies[self.idxN].get_pos()).arg())
        else:
            actions[self.idxN] = Actions.GoToPointIgnore(u , (field.ball.get_pos() - field.allies[self.idxN].get_pos()).arg())

    def block_pass(self, field: fld.Field, actions: list[Optional[Action]]) -> None:
        actions[self.idxN] = Actions.GoToPointIgnore((aux.point_on_line(field.allies[0].get_pos(), field.enemies[0].get_pos(), 300)) , (field.ball.get_pos() - field.allies[3].get_pos()).arg())

    def opening_to_the_ball(self, field: fld.Field, actions: list[Optional[Action]]) -> None:
        y = (field.ball.get_pos() - field.allies[self.idxN].get_pos()).unity()*50000
        b = -1
        for b in range(6):
            b+=1
            if(aux.dist(aux.closest_point_on_line(field.ball.get_pos(), field.allies[self.idxN].get_pos(), field.enemies[b].get_pos(), "S"), field.enemies[b].get_pos()) < 100):
                e = aux.get_angle_between_points(field.allies[self.idxN].get_pos(), field.ball.get_pos(), field.enemies[b].get_pos())
                if(e<0):
                    while(aux.dist(aux.closest_point_on_line(field.ball.get_pos(), field.allies[self.idxN].get_pos(), field.enemies[b].get_pos(), "S"), field.enemies[b].get_pos()) < 100):
                        actions[self.idxN] = Actions.GoToPointIgnore(aux.rotate(y, 3.14/2), (field.ball.get_pos() - field.allies[self.idxN].get_pos()).arg())
                else:
                    while(aux.dist(aux.closest_point_on_line(field.ball.get_pos(), field.allies[self.idxN].get_pos(), field.enemies[b].get_pos(), "S"), field.enemies[b].get_pos()) < 100):
                        actions[self.idxN] = Actions.GoToPointIgnore(aux.rotate(y, -3.14/2), (field.ball.get_pos() - field.allies[self.idxN].get_pos()).arg())
